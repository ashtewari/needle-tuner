...
{
    private static EvalProvider FakeProvider(string key, bool enabled = true) =>
        new(
            key: key,
            displayName: key,
            enabled: enabled,
    ...
    {
        var allProviders = new List<EvalProvider> { FakeProvider("needleBase"), FakeProvider("needleV3") };

        var ex = Assert.Throws<ArgumentException>(() =>
            Program.SelectProviders(allProviders, ["needleBase", "needleV3"]));

        Assert.Contains("needleV3", ex.Message, StringComparison.Ordinal);
    ...
    {
        var allProviders = new List<EvalProvider> { FakeProvider("needleMyTunedRun"), FakeProvider("needleV3") };
    ...
    {
        var allProviders = new List<EvalProvider> { FakeProvider("openAi"), FakeProvider("needleV3") };
